...
func _on_FeedbackDialog_about_to_show():
	...
	if Feedback.has_crash_to_report:
		...
		$Container/SendLogsCheckBox.button_pressed = true
	else:
		$Container/SendLogsCheckBox.button_pressed = false
...
func send_feedback(userName: String, description: String, logContents):
	...
	var postBody = "user_name=%s" % userName.uri_encode()
	postBody += "&description=%s" % description.uri_encode()
