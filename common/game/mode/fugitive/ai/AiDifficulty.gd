...
# Cells sampled when picking a hiding or fleeing spot: more is choosier
...
var detour_chance := 0.0
var detour_radius := 0.0
# Whether routes bend around cops the bot has seen
var keeps_clear_of_cops := true
# Whether the bot spends stamina running from a cop after the headstart
var sprints_to_flee := true
...
static func profile(level: int) -> AiDifficulty:
	...
	match level:
		...
		Level.HARD:
			...
			p.detour_chance = 0.0
			p.detour_radius = 0.0
			p.keeps_clear_of_cops = true
			p.sprints_to_flee = true
		_:
			p.speed_scale = 0.9
			p.think_interval = 0.2
			p.see_distance = 25.0
			p.caution_distance = 20.0
			p.panic_distance = 9.0
			p.crouch_distance = 14.0
			p.threat_memory = 5.0
			p.rescue_distance = 35.0
			p.safe_zone_dash_distance = 20.0
			p.spot_candidates = 40
			p.detour_chance = 0.2
			p.detour_radius = 10.0
			p.keeps_clear_of_cops = true
			p.sprints_to_flee = true
	return p
