...
func _enter_tree():
	ClientNetwork.connect("create_player", Callable(self, "create_player"))
	ClientNetwork.connect("update_player", Callable(self, "update_player"))
	ClientNetwork.connect("remove_player", Callable(self, "remove_player"))
	ClientNetwork.connect("game_data_updated", Callable(self, "update_game_data"))
	
	ClientNetwork.connect("lobby_countdown_started", Callable(self, "on_start_lobby_countdown"))
	ClientNetwork.connect("game_started", Callable(self, "on_start_game"))


func _exit_tree():
	ClientNetwork.disconnect("create_player", Callable(self, "create_player"))
	ClientNetwork.disconnect("update_player", Callable(self, "update_player"))
	ClientNetwork.disconnect("remove_player", Callable(self, "remove_player"))
	ClientNetwork.disconnect("game_data_updated", Callable(self, "update_game_data"))
	
	ClientNetwork.disconnect("lobby_countdown_started", Callable(self, "on_start_lobby_countdown"))
	ClientNetwork.disconnect("game_started", Callable(self, "on_start_game"))
...
func create_player_ui(playerId: int):
	...
	var playerNode = playerListItem.instantiate()
	playerNode.set_multiplayer_authority(playerId)
	playerNode.set_name(str(playerId))
	playerList.add_child(playerNode)
	
	playerNode.connect("make_host", Callable(self, "on_make_host"))
	playerNode.connect("kick_player", Callable(self, "on_kick_player"))
...
func update_host():
	...
	if host != null and host.get_id() == multiplayer.get_unique_id():
		is_host = true
	else:
		is_host = false
