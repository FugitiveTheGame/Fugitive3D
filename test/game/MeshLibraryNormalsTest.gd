...
# A tile whose node rotation was applied to its normals but not its positions
# lights only from one direction instead of facing up.
#
# Only tiles whose vertex positions still match source can be checked this
# way; where the transform is baked into positions too, rotated normals are
# correct.
const SOURCES := [
